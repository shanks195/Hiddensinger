...
function getListSong(page, limit) {
    var list = {};
    let url = '';
    if (page || limit) {
        url = `${PATH}api/song/list?page=${page}&&limit=${limit}`
    } else {
        url = `${PATH}api/song/list`
    }
...
function searchByNameSong(name, arrSongPage, arrSongAll) {
    if (name&&arrSongAll) {
        return arrSongAll.filter(function (item) {
...
function searchByNameSongg(name, arrSongAll) {
    if (name&&arrSongAll) {
        return arrSongAll.filter(function (item) {
            return item.name.toLowerCase().search(name.toLowerCase()) == -1 ? false : true;
        });
...
export { getListSong, getArrPageAble, searchByNameSong, searchByNameSongg };